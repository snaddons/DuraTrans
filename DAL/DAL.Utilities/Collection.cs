...
namespace DAL.Utilities
{
    public static class Collection
    {
        ...
        public static List<U> JJ_ToList<T, U>(IEnumerator collection, Converter<T, U> converter)
        {
            if (collection == null)
            {
                throw new ArgumentNullException("collection");
            }
            IEnumerator<U> newCollection = JJ_ListConvertAll(collection, converter);
            List<U> newList = new List<U>();
            while (newCollection.MoveNext())
            {
                newList.Add(newCollection.Current);
            }
            return newList;
        }

        public static IEnumerator<U> JJ_ListConvertAll<T, U>(IEnumerator iterator, Converter<T, U> converter)
        {
            if (iterator == null)
            {
                throw new ArgumentNullException("collection");
